...
def avg_mutant_death_rate_subfigure(stats, mutant_type, fig_name):
    site_baseline_rates = []
    site_rates = []
    pte_baseline_rates = []
    pte_rates = []
    for device in devices:
        site_baseline_rates.append(log(stats[SITE_BASELINE][device][mutant_type][AVG_RATE]))
        site_rates.append(log(stats[SITE][device][mutant_type][AVG_RATE]))
        pte_baseline_rates.append(log(stats[PTE_BASELINE][device][mutant_type][AVG_RATE]))
        pte_rates.append(log(stats[PTE][device][mutant_type][AVG_RATE]))

    log_labels = ["0", "1", "10", "100", "1,000", "10,000", "100,000"]
    x = np.arange(len(figure_labels))
    y = [0, 1, 10, 100, 1000, 10000, 100000]
    fig, ax = plt.subplots(1, 1, figsize=(5, 2))
    ax.bar(x - 1.5 * width, site_baseline_rates, width, color=SITE_BASELINE_COLOR)
    ax.bar(x - .5 * width, site_rates, width, color=SITE_COLOR)
    ax.bar(x + .5 * width, pte_baseline_rates, width, color=PTE_BASELINE_COLOR)
    ax.bar(x + 1.5 * width, pte_rates, width, color=PTE_COLOR)
